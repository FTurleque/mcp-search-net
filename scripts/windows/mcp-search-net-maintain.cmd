...
set "MAINTAIN_JS=%MCP_SEARCH_HOME%\app\build\cli\catalog-maintain.js"
if not exist "%NODE_EXE%" (
  1>&2 echo mcp-search-net: runtime Node.js absent. Relancez scripts\install-user.ps1.
  exit /b 2
)
if not exist "%MAINTAIN_JS%" (
  1>&2 echo mcp-search-net: CLI de maintenance absente. Relancez scripts\install-user.ps1.
  exit /b 3
)
