...
describe('residual audit persistence regressions', () => {
  it('retains sync-run ownership after a failed completion so the same owner can retry', async () => {
...
function readSyncRun(path: string, id: number): {
  ...
  try {
    const row = database
      .prepare<
        [number],
        { status: string; completed_at: number | null; owner_token: string | null }
...
function readVersionState(path: string, id: number): {
  ...
  try {
    const row = database
      .prepare<[number], { is_current: number; pending_current: number }>(
