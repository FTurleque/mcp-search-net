...
describe('file lease stale recovery cleanup', () => {
  it('keeps the active namespace available when stale quarantine and heartbeat cleanup are busy', () => {
    ...
    expect(JSON.parse(readFileSync(lockPath, 'utf8'))).toMatchObject({ ownerToken: 'new-owner' });
    ...
    expect(warnings.some((entry) => entry.message === 'file_lease_lock_stale_recovered')).toBe(true);
...
function recordingLogger(
  ...
  return {
    ...
    debug: () => undefined,
    info: () => undefined,
    warning: (message, data) => warnings.push({ message, ...(data === undefined ? {} : { data }) }),
    error: () => undefined,
  };
}
