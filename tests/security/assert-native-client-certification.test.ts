...
function assertContains(result: SpawnSyncReturns<string>, expected: string): void {
  ...
  // codes and a "     | " marker interposed, so a bare whitespace collapse is not enough; strip
  // the ANSI/marker noise first, then join wrapped lines with a single space.
